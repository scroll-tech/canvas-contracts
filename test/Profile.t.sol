...
contract ProfileRegistryTest is Test {
  ...
  function setUp() public {
    ...
    ERC1967Proxy profileRegistryProxy = new ERC1967Proxy(
      ...
      abi.encodeCall(ProfileRegistry.initialize, (TREASURY_ADDRESS, TREASURY_ADDRESS, address(profileImpl)))
    );
  ...
  function testReorderBadges(uint256 count, uint256 encoding) external {
    ...
    for (uint256 i = 0; i < count; ++i) {
      ...
      for (uint256 j = 0; j < count; ++j) {
        ...
        if (index == type(uint256).max || orders[j] < orders[index]) index = j;
      }
      orders[index] = i + 1;
      mask |= 1 << index;
    }
